...
const Dealer = () => {
  ...
  const get_dealer = async () => {
    ...
    if (retobj.status === 200) {
      setDealer(retobj.dealer); // Directly use the dealer object
    }
  };
  ...
  const get_reviews = async () => {
    ...
    console.log('Reviews API response:', retobj); // Log the response
    if (retobj.status === 200) {
  ...
  return (
    <div style={{ margin: "20px" }}>
      ...
      <div style={{ marginTop: "10px" }}>
        ...
        <h4 style={{ color: "grey" }}>
          {dealer?.city || 'City Not Available'}, {dealer?.address || 'Address Not Available'}, 
          Zip - {dealer?.zip || 'Zip Not Available'}, {dealer?.state || 'State Not Available'}
        </h4>
      </div>
